...
def render_projection(root: Path, target: Path, state: dict[str, Any], created_memory: bool) -> str:
    ...
    lines = [
        ...
        "",
        f"Source of truth: `{MEMORY_PATH.as_posix()}`.",
        style_lead(style),
        "",
        ...
        f"- Constitution: .specify/memory/constitution.md ({exists(root, '.specify/memory/constitution.md')})",
        f"- Architecture Memory: .specify/memory/architecture.md ({exists(root, '.specify/memory/architecture.md')})",
        f"- UC Memory: .specify/memory/uc.md ({exists(root, '.specify/memory/uc.md')})",
        f"- Feature Specs: {scan_feature_specs(root)}",
        ...
        "2. Repository constraints in this file, sourced from `.specify/memory/agent-governance.md`",
        "3. User-authored repository instructions outside Spec Kit managed markers",
        ...
        "9. Tool and MCP defaults",
        "",
        "## Non-Negotiable Execution Gates",
        "- Before editing implementation files, verify the active feature has spec.md, plan.md, and tasks.md under specs/<feature>/.",
        "- If any required feature artifact is missing, stop implementation and run the Spec Kit workflow before editing implementation files.",
        "- Do not treat bug fixes, refactors, or small code changes as exceptions to the spec/plan/tasks gate.",
